module.exports = {
    ...
    run: async(client, member) => {
        const {guild} = member
        if(!guild) return
        let db = client.antiraid.get(`raidmode_${guild.id}`) || {
            status: false,
            message: null,
            logs: null
        }
        if(db.status === true){
            ...
            try {
                ...
                client.antiraid.set(`raidmode_counter_${guild.id}`, parseInt(client.antiraid.get(`raidmode_counter_${guild.id}`) || 0) + 1);
            }
        }
    }
}