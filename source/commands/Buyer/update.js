  ...
  run: async (client, message, args) => {
    ...
    try {
      ...
      if (response.data.message === 'Mise à jour disponible en attente.') {
        ...
        exec(`cd /home/bot/${client.user.id} && rm -r source && rm version.js index.js package.json package-lock.json && cd /home/Update && cp -r * /home/bot/${client.user.id} && bun i  && pm2 restart ${client.user.id}`, async (err, stdout, stderr) => {
          if (err) {
            console.error(err);
