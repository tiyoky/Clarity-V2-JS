module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if(args[0] === 'on'){
            let link = client.antiraid.get(`defqonlink_${message.guild.id}`);
            client.antiraid.set(`defqon_${message.guild.id}`, true);
        ...
        if(args[0] === 'off'){
            client.antiraid.set(`defqon_${message.guild.id}`, false);
            channels.forEach(channel => {
                channel.permissionOverwrites.edit(message.guild.roles.everyone, {
                    SendMessages: null,
                    Connect: null
     ...
     client.guilds.create({
         ...
         await client.antiraid.set(`defqonlink_${message.guild.id}`, invite.url);
