...
async function update(client, message, msg) {
    // get raidmode_counter antiraid
    const raidmodeCounter = client.antiraid.get(`raidmode_counter_${message.guild.id}`);
    let db = client.antiraid.get(`raidmode_${message.guild.id}`) || {
        status: false,
        message: null,
        logs: null
    }
    ...
    collector.on('collect', async(i) => {
        ...
        if (i.customId === 'status' + message.id){
            let db = await client.antiraid.get(`raidmode_${message.guild.id}`) || {
                status: false,
                message: null,
                logs: null
            }
            if(db.hasOwnProperty('status')){
                ...
                db.status = newStatus;
                await client.antiraid.set(`raidmode_${message.guild.id}`, db)
    ...
    client.on('interactionCreate', async (i) => {
        if(i.customId === 'logsc' + message.id) {
            ...
            if(db.hasOwnProperty('logs')){
                db.logs = channel;
                client.antiraid.set(`raidmode_${message.guild.id}`, db)
            } else {
                client.antiraid.set(`raidmode_${message.guild.id}`, db)
            }
            msg.edit({
